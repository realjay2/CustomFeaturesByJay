...
-------------------------------------------------
-- Better Roads Toggle (Visuals Tab)
-------------------------------------------------
...
Tabs.Custom:Toggle({
	...
	Callback = function(enabled)
		...
		local function ApplyBetterRoads()
			...
			for _, obj in ipairs(workspace:GetDescendants()) do
				if obj:IsA("BasePart") then
					...
					local isRoadLine = name == "roadline" -- treated like road
					local isYellowLineUnderRoad = false

					-- Check if it's a yellow line inside a Road part
		...
		if enabled then
			local changed = ApplyBetterRoads()
