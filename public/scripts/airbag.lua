...
local function playAirSound(vehicle, mode)
	...
	if mode == "lower" then
		sound.SoundId = "rbxassetid://7768888198" -- air release (lower)
	elseif mode == "raise" then
		sound.SoundId = "rbxassetid://1" -- optional pump sound (replace if needed)
	else
		return
	end
...
UIS.InputBegan:Connect(function(input, gp)
	...
	if input.KeyCode == Enum.KeyCode.F then
		...
		-- Record original height first time
